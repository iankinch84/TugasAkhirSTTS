...
#pragma warning restore 0649
    ...
    public ClientHandle(TcpClient client) {
      this.ConnectionID = Guid.NewGuid().ToString().ToUpper();
      this.Connected = true;
    ...
    public void Disconnect() {
      ...
      if(OnDisconnected != null) { OnDisconnected.Invoke(this, new GEventArgs("Client Disconnected.")); }
    ...
    private void completeRead(IAsyncResult result) {
      ...
      if(bytesRead > 0) {
        ...
        if(data.StartsWith(_DISCONNECT)) {
          this.Connected = false;
          if(OnDisconnected != null) { OnDisconnected(this, new GEventArgs("Client Disconnected.")); }
    ...
    private void SendConnectionID() {
      this.Send(String.Format("{0}{1}", _AUTHENTICATE, this.ConnectionID));
    }
